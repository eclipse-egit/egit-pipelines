...
class Lib implements Serializable {
	 ...
	 *
	 * @return the URL for maven command line option "-Djgit-site=" or "-Degit-site="
	 */
	def String getMvnUpstreamRepo(String project, String version, String targetDirectory) {
		...
		script.sh(script: "mkdir -p \"${targetDirectory}/upstreamp2\" && curl -f -m 360 -L --max-redirs 8 -o \"${targetDirectory}/upstream.zip\" ${repo} && unzip -d \"${targetDirectory}/upstreamp2\" \"${targetDirectory}/upstream.zip\"")
		return "file://${targetDirectory}/upstreamp2"
	}
